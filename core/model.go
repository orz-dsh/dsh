...
// region modelConverter

type modelConverter[R any] interface {
...
func (h *modelHelper) NewError(rsn string, extra ...utils.DescKeyValue) error {
	kvs := KVS{
		reason(rsn),
		kv("file", h.File),
		kv("field", h.Field),
	}
	return utils.NewError(1, fmt.Sprintf("%s error", h.Title), append(kvs, extra...)...)
}

func (h *modelHelper) WrapError(err error, rsn string, extra ...utils.DescKeyValue) error {
	kvs := KVS{
		reason(rsn),
		kv("file", h.File),
		kv("field", h.Field),
	}
	return utils.WrapError(1, err, fmt.Sprintf("%s error", h.Title), append(kvs, extra...)...)
}

func (h *modelHelper) NewValueEmptyError() error {
	return utils.NewError(1, fmt.Sprintf("%s error", h.Title),
		reason("value empty"),
		kv("file", h.File),
		kv("field", h.Field),
	)
...
func (h *modelHelper) WrapValueInvalidError(err error, value any) error {
	return utils.WrapError(1, err, fmt.Sprintf("%s error", h.Title),
		reason("value invalid"),
		kv("file", h.File),
		kv("field", h.Field),
		kv("value", value),
...
func convertChildModels[R any, M modelConverter[R]](helper *modelHelper, field string, models []M) ([]R, error) {
	var result []R
