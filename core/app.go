...
func (a *App) MakeScripts(settings AppMakeScriptsSettings) (artifact *AppArtifact, err error) {
	...
	if outputPath == "" {
		...
		if settings.OutputPathClear {
			if err = utils.ClearDir(outputPath); err != nil {
				return nil, errW(err, "make scripts error",
	...
	if settings.Inspection {
		...
		if err = utils.WriteYamlFile(configsTracesInspectionPath, configsTraces); err != nil {
			return nil, errW(err, "make scripts error",
		...
		if err = utils.WriteYamlFile(dataInspectionPath, evaluator.ToMap(false)); err != nil {
			return nil, errW(err, "make scripts error",
		...
		if err = utils.WriteYamlFile(optionInspectionPath, a.context.option.inspect()); err != nil {
			return nil, errW(err, "make scripts error",
		...
		if err = utils.WriteYamlFile(profileInspectionPath, a.context.profile.inspect()); err != nil {
			return nil, errW(err, "make scripts error",
