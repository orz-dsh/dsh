...
func (a *AppArtifact) getTargetName(entity *executorItemSetting, targetGlob string) (targetName string, err error) {
	if targetGlob == "" {
...
func (a *AppArtifact) getExecutorArgs(setting *executorItemSetting, targetGlob string, targetName string, targetPath string) (executorArgs []string, err error) {
	args := setting.Args
	if len(args) == 0 {
